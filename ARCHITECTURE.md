...
- **Wordmark:** the real SVG wordmark is now on ALL seven pages, not just `index.html`. Each keeps its own treatment: solid `--text-0` on index/hub/share, **stroked with a cyan→violet SVG gradient** on `account.html` and `admin.html` (those used `background-clip:text` on live text, and this preserves that look with the real lettering), white on login's violet panel, and `#05070e` inside reset's gradient pill (where the "Portal" suffix stays as text — it is not part of the logo artwork).
- **SVG gradient gotcha (cost a real bug):** a `<linearGradient>` stroking the wordmark MUST use `gradientUnits="userSpaceOnUse"` with `x1="273.93" x2="1049.28"` (the wordmark's own viewBox span). With the default `objectBoundingBox` the gradient resolves PER PATH, and any path with a zero-width bounding box — the vertical "I", the straight stems — has a degenerate box and is **not painted at all**: the wordmark rendered as "MMUNOVCR'ɔC". Caught only by looking at it.
